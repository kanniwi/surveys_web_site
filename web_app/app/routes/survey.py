...
def uploads(filename):
    ...
    full_path = os.path.join(app_dir, 'uploads', filename)
...
def edit_survey(survey_id):
    ...
    if request.method == 'POST':
        try:
            ...
            if start_date_str:
                survey.start_date = datetime.strptime(start_date_str, "%Y-%m-%dT%H:%M").replace(tzinfo=timezone.utc)
            else:
                survey.start_date = datetime.now(timezone.utc)

            if end_date_str:
                survey.end_date = datetime.strptime(end_date_str, "%Y-%m-%dT%H:%M").replace(tzinfo=timezone.utc)
            else:
                survey.end_date = datetime(2999, 12, 31, tzinfo=timezone.utc)
