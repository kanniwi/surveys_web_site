...
class UserResponseRepository:
    ...
    def get_responses_by_user(self, user_id):
        return db.sesstion.query(UserResponse).filter_by(user_id=user_id).all()

    def get_responses_by_survey(self, survey_id):
        return db.sesstion.query(UserResponse).filter_by(survey_id=survey_id).all()

    def get_responses_by_question(self, question_id):
        return db.sesstion.query(UserResponse).filter_by(question_id=question_id).all()

    def get_response(self, user_id, question_id):
        return db.sesstion.query(UserResponse).filter_by(user_id=user_id, question_id=question_id).first()

    def delete_response(self, response_id):
        response = db.sesstion.query(UserResponse).filter_by(id=response_id).first()
        if response:
            db.sesstion.delete(response)
            db.sesstion.commit()
